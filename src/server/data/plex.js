...
const login = async (login, password) => {
    ...
    let { MediaContainer } = await plex.query(
        `/?X-Plex-Token=${admin.authToken}`
    );
    let { machineIdentifier } = MediaContainer;

    response = await fetch(
        `https://plex.tv/pms/resources?X-Plex-Token=${authToken}`,
        {
    ...
    let devices = response.children[0].children.filter(
        (child) => child.name == "Device"
    );
    let server = devices.find(
        (device) => device.attributes.clientIdentifier == machineIdentifier
    );
...
const getSections = async (user) => {
    ...
    let sections = response.MediaContainer.Directory.map(
        ({ uuid, title, type }) => ({
            uuid,
            title,
            type,
